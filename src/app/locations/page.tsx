...
export default function LocationsDirectoryPage() {
  ...
  const punjabCities = cities.filter((c) => c.region === 'Punjab' && !c.isHQ);
  const otherIndiaCities = cities.filter((c) => c.country === 'India' && c.region !== 'Punjab');
  const globalCities = cities.filter((c) => c.country !== 'India');
  ...
  return (
    <div className="w-full flex flex-col pt-12 sm:pt-20">
      ...
      {/* National & Global Hubs */}
      <SectionWrapper variant="warm">
        <div className="max-w-3xl space-y-2 mb-8">
          <span className="font-mono text-xs uppercase tracking-widest text-[#C99A44]">
            National &amp; International
          </span>
          <h2 className="font-serif text-3xl text-[#122C57]">Global Enterprise Hubs</h2>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...otherIndiaCities, ...globalCities].map((city) => (
            <Card
              key={city.citySlug}
              variant="outline"
              className="bg-[#FFFFFF] p-6 space-y-4 hover:border-[#122C57] transition-colors flex flex-col justify-between"
            >
              ...
              <Link
                ...
                className="inline-flex items-center gap-1.5 text-xs font-mono text-[#122C57] font-semibold hover:text-[#C99A44] pt-2 border-t border-[#E4E2DC]"
              >
                Explore {city.cityName} <ArrowRight className="w-3 h-3" />
              </Link>
            </Card>
          ))}
        </div>
